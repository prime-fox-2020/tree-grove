'use strict'

class FruitTree {
  
    constructor (name, age, height, matureAge,healthStatus) {
      this._name = name
      this._age = age
      this._height = height
      this._healthStatus = healthStatus
      this._matureAge = matureAge
    }
    get name(){
      return this._name
    }
    get age () {
      return this._age
    }
  
    get matureAge(){
        return this._matureAge
    }

    get height () {
      return this._height
    }
  
    get healthStatus () {
      return this._healthStatus
    }

    grow(){
        this._age += 1
        if (this._height > 8.75){
            this._height = this._height
        } else {
            this._height += Math.ceil(Math.random() * 3) * 0.375
        }
        if (this._age == this._matureAge + 10){
            this._healthStatus = false
        }
        return this
    }
}

class AppleTree extends FruitTree{
    constructor(name,age,height,matureAge,healthStatus){
        super(name,age,height,matureAge,healthStatus)
    }
}

// let appleTree = new AppleTree('apple',4,3,12,true)
  //  console.log('the apple tree is alive')
  //  do {
  //    appleTree.grow();
  //    console.log(`[Year ${appleTree.age} Report] Height = ${appleTree.height} Health = ${appleTree.healthStatus}`)
  //  } while (appleTree.healthStatus != false)
  //  console.log('the apple tree has dead')

module.exports = AppleTree